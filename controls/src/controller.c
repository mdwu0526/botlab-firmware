...
double run_controller(rc_filter_t** filters, motor_pid_params_t pid_params, motor_cmds_t setpoints, motor_states_t states, MotorFeedforward position_ff, MotorFeedforward velocity_ff, MotorFeedforward current_ff)
{
    ...
    double pwm_cmd = rc_filter_march(filters[2], states.i - (setpoints.i_sp + cur_cmd));
    if(current_ff != NULL)
    {
